...
public class StartTimeCalculator
{
    ...
    private readonly TimeSpan sunriseOffset;

    public StartTimeCalculator(double latitude, double longitude, TimeSpan sunriseOffset)
    {
    ...
    public DateTime GetStartTime(DateOnly date)
    {
        ...
        return GetSunrise(n) + this.sunriseOffset;
    }

    private DateTime GetSunrise(int n)
    {
        double meanSolarTime = n - (this.longitude / 360.0);
        ...
        double hourAngle = Acos((Sin(-0.833) - Sin(this.latitude) * Sin(declinationOfTheSun)) / Cos(this.latitude) * Cos(declinationOfTheSun));
